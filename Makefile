...
run: env-check ## Run the agent locally
	@echo "🤖 Starting medical-diagnostics-agent..."
	@uv run python -m medical_diagnostics_agent

.PHONY: run-dev
run-dev: ## Run the agent in development mode with auto-reload
	@echo "🤖 Starting medical-diagnostics-agent in development mode..."
	@uv run watchfiles "python -m medical_diagnostics_agent" medical_diagnostics_agent/
...
docker-build: ## Build Docker image
	...
	@docker build -f Dockerfile.agent -t medical_diagnostics_agent:latest .

.PHONY: docker-build-no-cache
docker-build-no-cache: ## Build Docker image without cache
	@echo "🐳 Building Docker image (no cache)..."
	@docker build --no-cache -f Dockerfile.agent -t medical_diagnostics_agent:latest .
...
docker-run: ## Run Docker container with .env file
	...
	@docker run -d \
		...
		medical_diagnostics_agent:latest
...
docker-stop: ## Stop Docker container
	...
	@docker stop medical_diagnostics_agent || true
	@echo "✅ Container stopped"

.PHONY: docker-rm
docker-rm: docker-stop ## Remove Docker container
	@echo "🗑️  Removing Docker container..."
	@docker rm medical_diagnostics_agent || true
	@echo "✅ Container removed"

.PHONY: docker-logs
docker-logs: ## View Docker container logs
	@docker logs -f medical_diagnostics_agent

.PHONY: docker-shell
docker-shell: ## Open shell in running Docker container
	@docker exec -it medical_diagnostics_agent bash
...
docker-clean: docker-rm ## Clean up Docker images and containers
	...
	@docker rmi medical_diagnostics_agent:latest || true
...
docker-push: ## Push Docker image to Docker Hub
	...
	@docker tag medical_diagnostics_agent:latest para5/medical_diagnostics_agent:latest
	@docker push para5/medical_diagnostics_agent:latest
...
status: ## Show agent and Docker status
	...
	@docker ps -a | grep medical_diagnostics_agent || echo "No containers found"
	@echo "\nDocker Images:"
	@docker images | grep medical_diagnostics_agent || echo "No images found"
	@echo "\nAgent Health:"
